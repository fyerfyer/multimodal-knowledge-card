...
class InferenceService:
    ...
    def get_model(self, model_type: Optional[str] = None) -> MultiModalModelInterface:
        ...
        try:
            ...
            elif model_type == ModelType.DEEPSEEK_VL.value or model_type.startswith("deepseek"):
                model = DeepSeekVLModel(
                    api_key=settings.DEEPSEEK_API_KEY,
                    model_name=settings.LLM_MODEL_PATH or "deepseek-vl"
                )
            else:
    ...
    def switch_model(self, model_type: str) -> bool:
        ...
        try:
            ...
            # 检查是否支持该模型类型
            if not (model_type == ModelType.QWEN_VL.value or 
                   model_type == ModelType.DEEPSEEK_VL.value or
                   model_type.startswith("qwen") or 
                   model_type.startswith("deepseek")):
    ...
    def list_available_models(self) -> Dict[str, Dict[str, Any]]:
        ...
        # 添加未加载但支持的模型
        for model_type in [ModelType.QWEN_VL.value, ModelType.DEEPSEEK_VL.value]:

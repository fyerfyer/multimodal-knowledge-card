...
from app.core.models.deepseek_vl import DeepSeekVLModel
...
class TestMultiModalModels(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.deepseek_api_key = os.environ.get("DEEPSEEK_API_KEY") or settings.DEEPSEEK_API_KEY
        
        if not cls.dashscope_api_key:
            logger.warning("DASHSCOPE_API_KEY not set, some tests will be skipped")
            
        if not cls.deepseek_api_key:
            logger.warning("DEEPSEEK_API_KEY not set, some tests will be skipped")
    ...
    def test_deepseek_vl_model(self):
        """测试DeepSeek视觉语言模型"""
        if not self.deepseek_api_key:
            self.skipTest("DEEPSEEK_API_KEY not available")
            
        # 创建模型实例
        model = DeepSeekVLModel(api_key=self.deepseek_api_key)
        ...
        # 测试获取模型信息
        model_info = model.get_model_info()
        self.assertEqual(model_info["model_type"], "DeepSeek-VL")
        self.assertTrue(model_info["supports_vision"])
        
        # 测试生成响应（仅测试文本输入）
        response = model.generate_response(
            prompt="深度学习是什么？请用通俗易懂的语言解释。",
            image=None,
            options={"max_tokens": 100}
        )
    ...
    def test_model_factory(self):
        ...
        if self.deepseek_api_key:
            deepseek_model = create_model("deepseek-vl", api_key=self.deepseek_api_key)
            self.assertIsInstance(deepseek_model, DeepSeekVLModel)
            self.assertTrue(deepseek_model.is_initialized())
        
        # 测试模型类型列表
        available_models = ModelFactory.list_available_models()
        self.assertIn("qwen-vl", available_models)
        self.assertIn("deepseek-vl", available_models)
        
        # 测试注册和获取新模型类
        class TestModel(MultiModalModelInterface):
            def initialize(self): return True
            def generate_response(self, prompt, image=None, options=None): return {}
            def get_model_info(self): return {"name": "test"}
    ...
    def test_model_loader(self):
        ...
        # 选择可用的API密钥来进行测试
        api_key = self.dashscope_api_key or self.deepseek_api_key
        model_type = "qwen-vl" if self.dashscope_api_key else "deepseek-vl"
